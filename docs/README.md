...
Next, we create the base `Deployment` for the application and register the resource with the Kustomization.
```yaml
# add this to a new microservice-deployment.yaml file
---
apiVersion: apps/v1
kind: Deployment
metadata:
...
# add this to a new microservice-service.yaml file
---
apiVersion: v1
kind: Service
metadata:
...
# add these to the kustomization.yaml
---
commonLabels:
  app: weather-service
  framework: golang
...
Add the following definitions to the `kustomization.yaml`
```yaml
configMapGenerator:
  - name: database-configmap
    literals:
